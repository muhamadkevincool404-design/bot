...
api_id = 'MASUKAN_API_ID'
api_hash = 'MASUKAN_API_HASH'
...
@client.on(events.NewMessage(pattern='.gb', outgoing=True))
...
@client.on(events.NewMessage(pattern='.no', outgoing=True))
async def blacklist_group(event):
    ...
    if group_id not in blacklisted_groups:
        ...
        await event.respond(append_watermark_to_message("🚫 Group sudah berhasil di skip."))
    else:
...
async def show_help(event):
    help_text = (
        "🛠 **Available Commands:**\n"
        ".gb - Promote a message to all groups.\n"
        ".no - Blacklist the current group from receiving promotions.\n"
